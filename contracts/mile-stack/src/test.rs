...
// ---------------------------------------------------------------------------
// Issue #4 — approve_milestone tests
// ---------------------------------------------------------------------------
...
fn test_approve_milestone_releases_xlm_to_freelancer() {
    ...
    // After funding: escrow holds the XLM, freelancer has nothing yet.
    assert_eq!(token_client.balance(&contract_id), milestone_amount);
    assert_eq!(token_client.balance(&freelancer), 0);

    contract.approve_milestone(&project_id, &0, &token_address);

    // After approval: escrow is drained, freelancer received the XLM.
    assert_eq!(token_client.balance(&contract_id), 0);
...
fn test_approve_milestone_rejects_pending_milestone() {
    ...
    // Attempt to approve without funding first must panic.
    contract.approve_milestone(&project_id, &0, &token_address);
}
...
fn test_approve_milestone_rejects_already_released_milestone() {
    ...
    contract.fund_milestone(&project_id, &0, &token_address);
    contract.approve_milestone(&project_id, &0, &token_address);
    // Second approval on a Released milestone must panic.
    contract.approve_milestone(&project_id, &0, &token_address);
}

// ---------------------------------------------------------------------------
// Issue #5 — view function tests
// ---------------------------------------------------------------------------
...
fn test_view_functions_do_not_require_auth() {
    // env.auths() returns the auth entries recorded during the LAST invocation.
    // If a function calls require_auth(), its entry appears here.
    // View functions have no require_auth() — so auths() must be empty after each.
    let env = make_env();
    env.mock_all_auths();
...
fn test_get_milestone_panics_for_out_of_range_index() {
    ...
    // Project has 1 milestone (index 0); index 1 must panic.
    contract.get_milestone(&project_id, &1);
}

// ---------------------------------------------------------------------------
// Issue #6 — dispute_milestone tests
// ---------------------------------------------------------------------------
...
fn test_dispute_milestone_locks_funds_in_contract() {
    ...
    // XLM is in escrow after funding.
    assert_eq!(token_client.balance(&contract_id), milestone_amount);

    contract.dispute_milestone(&client_addr, &project_id, &0);

    // XLM must remain locked — dispute does not move funds.
    assert_eq!(token_client.balance(&contract_id), milestone_amount);
    assert_eq!(token_client.balance(&freelancer), 0);
    assert_eq!(token_client.balance(&client_addr), 0);
}
...
fn test_dispute_milestone_does_not_affect_siblings() {
    ...
    // Fund only milestone 0; milestone 1 stays Pending.
    let token_address = setup_token(&env, &token_admin, &client_addr, 500_0000000);
    let project_id = funded_project(&env, &contract, &client_addr, &freelancer, &token_address);
...
fn test_dispute_milestone_rejects_pending_milestone() {
    ...
    // Create project but do NOT fund — milestone stays Pending.
    let project_id = create_demo_project(&env, &contract, &client_addr, &freelancer);
...
fn test_dispute_milestone_rejects_already_released_milestone() {
    ...
    // Approve (releases payment) then try to dispute — must panic.
    contract.approve_milestone(&project_id, &0, &token_address);
    contract.dispute_milestone(&client_addr, &project_id, &0);
}
